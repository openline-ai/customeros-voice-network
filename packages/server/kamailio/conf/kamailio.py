...
class kamailio:
    ...
    def ksr_route_transfer_invite(self, msg):
        ...
        if re.search("^[+]?[0-9]+$", user) is not None:
            ...
            if record is None:
                ...
                return -255
            profile = self.kamailioDB.find_profile(record['profile_id'])
            if profile is not None:
                KSR.hdr.append("X-Openline-Profile-Info: %s\r\n" % json.dumps(profile))
    ...
    def ksr_route_from_asterisk(self, msg):
        ...
        if KSR.pv.get("$hdr(X-Openline-Dest-Carrier)") is not None:
    ...
    def ksr_route_from_webrtc(self, msg):
        ...
        if KSR.pv.gete("$rU") == "echo":
            ...
            self.log_info("Number found, checking if PSTN is activated\n")
            return self.ksr_route_to_carrier_via_asterisk(from_url, False, msg)
        else:
            self.log_info("Destination not a number nor is registered")
            KSR.tm.t_send_reply(404, "Destination Not Found")
            return -255

    def ksr_route_to_carrier_via_asterisk(self, from_url, call_forwarding, msg):
        self.log_info(
            "Looking up %s in database\n" % from_url)
        ...
        #Inbound call forwarding to phoneuri
        if from_url != record['sipuri'] and call_forwarding is False:
            KSR.hdr.append("X-Openline-User: " + record['sipuri'] + "\r\n")
        elif call_forwarding is True:
            # Call forwarding, of inbound call, Dest-User is the original destination Dest is the new destination
            KSR.hdr.append("X-Openline-Dest-User: " + from_url + "\r\n")
        ...
        if profile is not None:
            KSR.hdr.append("X-Openline-Profile-Info: %s\r\n" % json.dumps(profile))
        return self.ksr_route_asterisk(msg, False)
    ...
    def ksr_route_from_carrier(self, msg):
        ...
        if result['forwarding_enabled'] is not None and result['forwarding_enabled'] == True:
            self.log_info("Number is forwarded to %s\n" % result['forwarding_e164'])
            KSR.pv.sets("$rU", result['forwarding_e164'])
            return self.ksr_route_to_carrier_via_asterisk(result['sipuri'], True, msg)

        profile = self.kamailioDB.find_profile(result['profile_id'])
        if profile is not None:
            KSR.hdr.append("X-Openline-Profile-Info: %s\r\n" % json.dumps(profile))

        voicemail = self.kamailioDB.find_voicemail(result['voicemail_id'])
        if voicemail is not None:
            KSR.tm.t_on_failure("ksr_failure_voice_mail")
            KSR.tm.t_set_fr(voicemail["timeout"]*1000, 0)
            KSR.pv.sets("$avp(voicemail_user)", result['sipuri'])
            KSR.hdr.append("X-Openline-Voicemail-Info: %s\r\n" % json.dumps(voicemail))
    ...
    def ksr_route_asterisk(self, msg, fork=False):
        ...
        if not fork:
            ...
            if KSR.pv.gete("$rU") != "echo"  and  KSR.pv.gete("$rU") != "voicemail":
            ...
            self.log_info("ksr_route_asterisk: Forking to %s" % orig_dest)
    ...
    def ksr_failure_voice_mail(self, msg):
        self.log_info("All destinations failed, routing to voicemail\n")
        KSR.pv.sets("$rU", "voicemail")
        KSR.pv.sets("$avp(uuid)", KSR.pv.get("$avp(uuid)") + "-vm")
        KSR.hdr.append("X-Openline-Dest-User: " + KSR.pv.gete("$avp(voicemail_user)") + "\r\n")
        return self.ksr_route_asterisk(msg, False)
    ...
    def cleanup_headers(self, msg):
        ...
        KSR.hdr.remove("X-Openline-Profile-Info")
        KSR.hdr.remove("X-Openline-Voicemail-Info")
