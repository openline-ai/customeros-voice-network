...
class MyTestCase(unittest.TestCase):
    ...
    def test_INVITE_from_webrtc_to_pstn(self):
        ksr_utils.pvar_set("$si", "1.2.3.4") #random ip address on public network
        ksr_utils.pvar_set("$rm", "INVITE")
        ...
        def mock_find_profile(profile_id: int):
            self.assertEqual(profile_id, 5, "Incorrect profile id passed to find_profile")
            return {"profile_name": "test_profile",
                        "call_webhook": "http://call_webhook",
                        "recording_webhook": "http://recording_webhook",
                        "api_key": "awesome_api_key",
                        }

        k.kamailioDB._mock['find_profile'] = mock_find_profile
        ...
        self.assertIsNotNone(ksr_utils.pvar_get("$hdr(X-Openline-Profile-Info)"), "Missing Profile Header")
        profile = json.loads(ksr_utils.pvar_get("$hdr(X-Openline-Profile-Info)"))
        self.assertEqual(profile["profile_name"], "test_profile", "Profile name not set correctly")
        self.assertEqual(profile["call_webhook"], "http://call_webhook", "Call webhook not set correctly")
        self.assertEqual(profile["recording_webhook"], "http://recording_webhook", "Recording webhook not set correctly")
    ...
    def test_INVITE_from_esim_to_pstn(self):
        ksr_utils.pvar_set("$si", "1.2.3.4") #random ip address on public network
        ksr_utils.pvar_set("$rm", "INVITE")
        ksr_utils.pvar_set("$ru", "sip:+4475755588858@openline.ai")
        ksr_utils.pvar_set("$fu", "sip:MyAwesomeESIM@agent.openline.ai")
        ksr_utils.pvar_set("$ct", "<sip:10.0.0.2:8080>")
        ...
        def mock_sipuri_mapping(sipuri: str):
            self.assertEqual(sipuri, "sip:MyAwesomeESIM@agent.openline.ai", "Incorrect key lookup in database")
            return {"e164": '+328080970',
                    "alias": '+322800000',
                    "carrier": 'test_carrier',
                    "sipuri": 'sip:AgentSmith@agent.openline.ai',
        ...
        self.assertEqual(ksr_utils.pvar_get("$ru"), "sip:transcode@openline.ai",
                         "RURI not set to expected destination")  # add assertion here
        self.assertEqual(ksr_utils.pvar_get("$nh(u)"), "sip:dispatcher_group_0",
                         "Dispatcher not invoked to right dispatcher group")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Endpoint-Type)"), "pstn", "Endpoint Header not set")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-Endpoint-Type)"), "pstn",
                         "Dest Endpoint Header not set")
        ...
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-CallerID)"), "+322800000", "CallerID not SET!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-Carrier)"), "test_carrier", "Carrier not SET!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-User)"), "sip:AgentSmith@agent.openline.ai", "X-Openline-User not set correctly!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest)"), "sip:+4475755588858@openline.ai",
                         "X-Openline-Dest not set!")
        self.assertTrue(ksr_relay_called, "Call was not routed!")
    ...
    def test_INVITE_from_pstn_to_webrtc(self):
        ...
        def mock_e164_mapping(e164:str, carrier:str):
            ...
            return {"sipuri": 'sip:AgentSmith@agent.openline.ai',
                    ...
                    "forwarding_e164": None,
                    }
        k.kamailioDB._mock['find_e164_mapping'] = mock_e164_mapping

    def test_INVITE_from_pstn_to_webrtc_vm(self):
        ksr_utils.pvar_set("$rm", "INVITE")
        ksr_utils.pvar_set("$ru", "sip:+328080970@openline.ai")
        ksr_utils.pvar_set("$fu", "sip:+44075755588858@agent.openline.ai")
        ksr_utils.pvar_set("$ct", "<sip:10.0.0.2:8080>")
        ...
        def my_t_set_fr(timeout: int, timeout2: int):
            nonlocal ksr_t_set_fr_called
            ksr_t_set_fr_called = True
            self.assertEqual(timeout, 10*1000)
            return 1

        KSR._mock_data["tm"]["t_set_fr"] = my_t_set_fr
        ...
        def mock_find_voicemail(voicemail_id: int):
            self.assertEqual(voicemail_id, 9, "Incorrect voicemail id passed to find_voicemail")
            return {"prompt_object_id": "this_is_my_voicemail_prompt",
                    "timeout": 10,
                        }

        k.kamailioDB._mock['find_voicemail'] = mock_find_voicemail

        k.ksr_request_route(None)
        k.ksr_failure_voice_mail(None)

        self.assertEqual(ksr_utils.pvar_get("$rU"), "voicemail", "RURI not set to expected destination")  # add assertion here
        self.assertEqual(ksr_utils.pvar_get("$nh(u)"),"sip:dispatcher_group_0", "Dispatcher not invoked to right dispatcher group")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Endpoint-Type)"),"pstn", "Endpoint Header not set")
        ...
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest)"), "sip:voicemail@dispatcher_group_0", "X-Openline-Dest not set!")
        self.assertTrue(ksr_relay_called, "Call was not routed!")

        self.assertTrue(ksr_t_set_fr_called, "t_set_fr was not called!")


    def test_INVITE_from_pstn_to_pstn(self):
        ksr_utils.pvar_set("$rm", "INVITE")
        ksr_utils.pvar_set("$ru", "sip:+328080970@openline.ai")
        ksr_utils.pvar_set("$fu", "sip:+44075755588858@agent.openline.ai")
        ksr_utils.pvar_set("$ct", "<sip:10.0.0.2:8080>")
        ...
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-CallerID)"), "+322800000", "CallerID not SET!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-Carrier)"), "test_carrier", "Carrier not SET!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-User)"), "sip:AgentSmith@agent.openline.ai", "X-Openline-User not set correctly!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest)"), "sip:+4475755588858@openline.ai",
                         "X-Openline-Dest not set!")
        self.assertTrue(ksr_relay_called, "Call was not routed!")

    def test_INVITE_from_pstn_to_esim(self):
        ksr_utils.pvar_set("$rm", "INVITE")
        ksr_utils.pvar_set("$ru", "sip:+328080970@openline.ai")
        ksr_utils.pvar_set("$fu", "sip:+44075755588858@agent.openline.ai")
        ksr_utils.pvar_set("$ct", "<sip:10.0.0.2:8080>")
        ...
        ksr_relay_called = False
        ksr_branch_called = False
        ksr_append_branch_called = False
        def allow_source_address(mode: int):
            ksr_utils.pvar_set("$avp(carrier)", "test_carrier")
            return 1

        def append_branch() -> int:
            nonlocal ksr_append_branch_called
            nonlocal ksr_branch_called
            if ksr_branch_called:
                k.ksr_branch_manage(None)
            self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest)"), "sip:AgentSmith@agent.openline.ai",
                             "X-Openline-Dest not set!")
            self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-Endpoint-Type)"), "webrtc",
                             "Endpoint Header not set")
            ksr_append_branch_called = True
            return 1


        def my_t_on_branch(name: str):
            self.assertEqual(name, "ksr_branch_manage", "Incorrect branch handler set")
            nonlocal ksr_branch_called
            ksr_branch_called = True
            return 1
        ...
        #indicate the call is not coming from an asterisk ip
        KSR._mock_data["dispatcher"]["ds_is_from_list"] = 0
        KSR._mock_data["corex"]["append_branch"] = append_branch
        KSR._mock_data["tm"]["t_on_branch"] = my_t_on_branch
        ...
        def mock_e164_mapping(e164:str, carrier:str):
            ...
            return {"sipuri": 'sip:AgentSmith@agent.openline.ai',
                     "phoneuri": 'sip:MyAwesomeESIM@agent.openline.ai',
                    "profile_id": 5,
                    "voicemail_id": None,
                    "forwarding_enabled": None,
                    "forwarding_e164": None,
        ...
        self.assertTrue(ksr_append_branch_called, "Apprend branch not called")
        ...
        self.assertEqual(ksr_utils.pvar_get("$nh(u)"),"sip:dispatcher_group_0", "Dispatcher not invoked to right dispatcher group")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Endpoint-Type)"),"pstn", "Endpoint Header not set")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest-Endpoint-Type)"),"pstn", "Endpoint Header not set")
        ...
        self.assertIsNotNone(ksr_utils.pvar_get("$hdr(X-Openline-UUID)"), "Missing UUID Header")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Origin-Carrier)"), "test_carrier", "Carrier not SET!")
        self.assertEqual(ksr_utils.pvar_get("$hdr(X-Openline-Dest)"), "sip:MyAwesomeESIM@agent.openline.ai", "X-Openline-Dest not set!")
        self.assertTrue(ksr_relay_called, "Call was not routed!")
    ...
    def test_INVITE_from_asterisk_transfer_to_pstn(self):
        ...
        self.assertIsNone(ksr_utils.pvar_get("$hdr(X-Openline-Profile-Info)"), "Sensitive headers blead through")
    ...
    def test_INVITE_from_asterisk_forward_to_pstn(self):
        ksr_utils.pvar_set("$rm", "INVITE")
        ksr_utils.pvar_set("$ru", "sip:127.0.0.1")
        ksr_utils.pvar_set("$fu", "sip:+328080970@agent.openline.ai")
        ksr_utils.pvar_set("$ct", "<sip:10.0.0.2:8080>")
        ...
        ksr_utils.registrations["kamailio_location"]["sip:+44075755588858@agent.openline.ai"] = "sip:should_not_be_here"
